...
// Static variables.
Test.Builder.globalScope = typeof self != 'undefined'
  ? self
  : typeof _global != 'undefined'
    ? _global
    : null;

Test.Builder.VERSION = '0.11_1';
...
Test.Builder.prototype._setupOutput = function () {
    if (Test.PLATFORM == 'browser') {
        var writer = function (msg) {
            ...
            var node = Test.Builder.globalScope.document.getElementById("test");
            if (node) {
                ...
                for (var i = 0; i < node.childNodes.length; i++) {
                    if (node.childNodes[i].nodeType == 3 /* Text Node */) {
                        ...
                        Test.Builder.globalScope.scrollTo(0, Test.Builder.globalScope.document.body.offsetHeight
                                        || Test.Builder.globalScope.document.body.scrollHeight);
                        return;
                    }
                }

                // If there was no text node, add one.
                node.appendChild(Test.Builder.globalScope.document.createTextNode(msg));
                Test.Builder.globalScope.scrollTo(0, Test.Builder.globalScope.document.body.offsetHeight
                                || Test.Builder.globalScope.document.body.scrollHeight);
                return;
            }
            ...
            Test.Builder.globalScope.document.write(msg);
            Test.Builder.globalScope.scrollTo(0, Test.Builder.globalScope.document.body.offsetHeight
                            || Test.Builder.globalScope.document.body.scrollHeight);
        };
        ...
        if (Test.Builder.globalScope) {
            if (Test.Builder.globalScope.alert.apply) this.warnOutput(Test.Builder.globalScope.alert, Test.Builder.globalScope);
            else this.warnOutput(function (msg) { Test.Builder.globalScope.alert(msg) });
        }
...
Test.Builder.prototype._notifyHarness = function () {
    ...
    if (Test.Builder.globalScope.parent
        && Test.Builder.globalScope.parent.Test && Test.Builder.globalScope.parent.Test.Harness) {
        Test.Builder.globalScope.parent.Test.Harness.Done++;
    }
};
...
if (Test.Builder.globalScope) {
    alert("Setup: " + Test.Builder.globalScope.name);
    if (Test.Builder.globalScope.closed) alert("Closed");
    // Set up an onload function to end all tests.
    Test.Builder.globalScope.onload = function () {
        try {
            alert("Onload: " + this.name);
            for (var i = 0; i < this.Test.Builder.Instances.length; i++) {
                // The main process is always async ID 0.
                this.Test.Builder.Instances[i].endAsync(0);
            }
        } catch (e) {
            alert("Catch: " + this.name);
	    throw e;
        }
    };
    ...
    Test.Builder.globalScope.onerror = function (msg, url, line) {
        ...
        this.Test.Builder.Test.TestDied = true;
        this.Test.Builder.Test.diag("Error in " + url + " at line " + line + ": " + msg);
        return true;
    };
};
...
Test.Builder.prototype.beginAsync = function (timeout) {
	var id = ++this.asyncID;
    if (timeout && Test.Builder.globalScope && Test.Builder.globalScope.setTimeout) {
        // Are there other ways of setting timeout in non-browser settings?
        var aTest = this;
        this.asyncs[id] = Test.Builder.globalScope.setTimeout(
