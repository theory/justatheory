...
Test.Harness.Browser = function () {
    this.includes = arguments.length ? arguments : [];
};

Test.Harness.Browser.VERSION = '0.11_1';
...
Test.Harness.Browser.prototype.runTest = function (file, buffer) {
    if (/\.html$/.test(file)) {
        ...
        // Using doc.write() works in virtually any browser.
        doc.write('<html><head>');
        for (var i = 0; i < this.includes.length; i++) {
            doc.write(
                '<script type="text/javascript" src="'
                + this.includes[i] + '"></script>'
            );
        }
        doc.write(
            '</head><body><pre id="test">'
    ...
    if (Test.Harness.Browser.prototype.args[key] == null) {
        ...
        if (typeof Test.Harness.Browser.prototype.args[key] == 'string') {
            Test.Harness.Browser.prototype.args[key] =
                [Test.Harness.Browser.prototype.args[key], unescape(val)];
        } else Test.Harness.Browser.prototype.args[key].push(unescape(val));
    }
}
delete pairs;
